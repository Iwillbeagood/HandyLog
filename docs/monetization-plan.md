...
### Android — Product Flavor (Free / Paid)

두 개의 flavor로 **별도 앱**으로 배포한다.

| 항목 | free | paid |
|------|------|------|
| applicationId | `com.hand.log` | `com.hand.log.pro` |
| 앱 이름 | HandyLog | HandyLog Pro |
| 기능 제한 | 테이블 3개, 핸드 30개/테이블, 플레이어 5명 등 | 모든 기능 해제 |
| 결제 | IAP로 Pro 업그레이드 (선택) | 불필요 |
| 스토어 | 무료 앱으로 등록 | 유료 앱으로 등록 |
...
```kotlin
// core/common 또는 domain에서 접근
expect fun isProBuild(): Boolean

// androidMain
actual fun isProBuild(): Boolean = BuildConfig.IS_PRO

// 사용처 (CheckFeatureLimitUseCase)
// isProBuild()가 true면 모든 제한 해제
// false면 ProRepository의 구매 상태에 따라 판단
```

- **free flavor**: 기능 제한 적용 + IAP로 인앱 업그레이드 가능
- **paid flavor**: `isProBuild() = true` → 모든 제한 해제, 결제 모듈 불필요
- paid flavor에서는 Paywall UI, BillingClient 의존성을 제외하여 앱 크기 최소화 가능

### iOS — 유료 앱 단일 배포

iOS는 유료 앱으로만 배포한다. 무료 버전 없음.

| 항목 | 값 |
|------|-----|
| Bundle ID | `com.hand.log.pro` |
| 앱 이름 | HandyLog |
| 가격 | $6.99~$14.99 (App Store 유료 앱) |
| 기능 제한 | 없음 — 모든 기능 해제 |
| IAP | 불필요 |

```kotlin
// iosMain
actual fun isProBuild(): Boolean = true  // 항상 Pro
```

- 구매 복원 버튼 불필요 (IAP 없으므로)
- App Store 심사 시 유료 앱 가격 정책만 설정

### 플랫폼별 요약

| | Android Free | Android Paid | iOS |
|---|---|---|---|
| 배포 형태 | 무료 앱 | 유료 앱 | 유료 앱 |
| 패키지/번들 | `com.hand.log` | `com.hand.log.pro` | `com.hand.log.pro` |
| 기능 제한 | O (Freemium) | X | X |
| IAP | O (Pro 업그레이드) | X | X |
| BillingClient | 필요 | 불필요 | 불필요 |
| Paywall UI | 필요 | 불필요 | 불필요 |
